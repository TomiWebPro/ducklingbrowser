...
Every session gets its own temporary Duckling data/cache/log root, home directory,
...
- `402` — reserved (not currently used; all features are free with no paid gatekeeping). Previously used for automation endpoints and expired-proxy checks before the removal of subscription gating.
...
`src-tauri/src/app_dirs.rs::app_name()` returns `"DucklingBrowserDev"` when `cfg!(debug_assertions)` is true, `"DucklingBrowser"` otherwise. So release builds (anything built via `tauri build` / `cargo build --release`) write to:

- macOS — `~/Library/Application Support/DucklingBrowser/`
- Linux — `~/.local/share/DucklingBrowser/`
...
The `scripts/publish-repo.sh` script publishes DEB and RPM packages to Cloudflare R2 (served at `repo.ducklingbrowser.com`). It requires Linux tools, so run it in Docker on macOS:
...
Sync mirrors local state to S3-compatible storage (Duckling cloud, or a self-hosted
`duckling-sync` NestJS server). Two distinct mechanisms live in `src-tauri/src/sync/`:
...
### Server (`duckling-sync/`) metadata passthrough
...
`duckling-sync/src/sync/dto/sync.dto.ts`; logic: `sync.service.ts`.

## Open Source

This project is licensed under AGPL-3.0. All features are free and unlocked by default — there is no subscription or paid gatekeeping. This is a community fork of the original Donut Browser with all subscription gating removed. See [TomiWebPro/ducklingbrowser](https://github.com/TomiWebPro/ducklingbrowser).
