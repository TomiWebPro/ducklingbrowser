{
  description = "Duckling Browser development environment and quick-start commands";
      ...
      let
        ...
        releaseAppImage =
          if system == "x86_64-linux" then
            pkgs.fetchurl {
              url = "https://github.com/TomiWebPro/ducklingbrowser/releases/download/v0.28.2/Duckling_0.28.2_amd64.AppImage";
              hash = "sha256-+CqHiPMg4oczNiPg+MC6jvp0CUcK4kb5yeyk+QDbAWY=";
            }
          else if system == "aarch64-linux" then
            pkgs.fetchurl {
              url = "https://github.com/TomiWebPro/ducklingbrowser/releases/download/v0.28.2/Duckling_0.28.2_aarch64.AppImage";
        ...
        releaseUnpacked =
          if releaseAppImage != null then
            pkgs.stdenvNoCC.mkDerivation {
              pname = "duckling-release-unpacked";
              ...
              installPhase = ''
                ...
                cp "$src" ./duckling.AppImage
                chmod +x ./duckling.AppImage
                ./duckling.AppImage --appimage-extract >/dev/null
            ...
            pkgs.appimageTools.wrapType2 {
              ...
              extraInstallCommands = ''
                for bin in "$out"/bin/*; do
                  if [ -f "$bin" ]; then
                    mv "$bin" "$out/bin/duckling-release"
                    break
                  fi
                done
              '';
          ...
          if releaseUnpacked != null then
            pkgs.writeShellApplication {
              ...
              text = ''
                ...
                if [ -x "${releaseWrapped}/bin/duckling-release" ]; then
                  if "${releaseWrapped}/bin/duckling-release" "$@"; then
                    exit 0
                  fi
      ...
      in
      {
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            echo "Duckling Browser dev shell ready."
            echo "Quick start:"
            echo "  nix run .#setup"
        ...
        apps.deps = mkApp "duckling-deps" ''
          set -euo pipefail
          pnpm install
        '';

        apps.dev = mkApp "duckling-dev" ''
          set -euo pipefail
          pnpm dev
        '';

        apps."tauri-dev" = mkApp "duckling-tauri-dev" ''
          set -euo pipefail
          pnpm tauri dev
        '';

        apps."full-dev" = mkApp "duckling-full-dev" ''
          set -euo pipefail
          chmod +x ./scripts/dev.sh
          ./scripts/dev.sh
        '';

        apps.build = mkApp "duckling-build" ''
          set -euo pipefail
          pnpm build
          (cd src-tauri && cargo build)
        '';

        apps.start = mkApp "duckling-start" ''
          set -euo pipefail
          pnpm start
        '';

        apps.test = mkApp "duckling-test" ''
          set -euo pipefail
          pnpm format && pnpm lint && pnpm test
        '';

        apps.setup = mkApp "duckling-setup" ''
          set -euo pipefail

          if [ ! -f "package.json" ]; then
            echo "package.json not found. Run this from the ducklingbrowser repo root."
            exit 1
          fi
        ...
        apps."release-start" = {
          type = "app";
          program = "${releaseLauncher}/bin/duckling-release-start";
        };
