...
import { launchDucklingClone } from "@/lib/duckling-physics";
...
// Flywheel: each click adds spin; past this speed the duckling escapes the
  ...
  const spinFrame = useCallback(
    (time: number) => {
      ...
      if (velocityRef.current >= ESCAPE_VELOCITY) {
        // The flywheel wins: the duckling tears loose and joins the bounce sim,
        // keeping its spin.
        stopSpin();
        setLogoFlown(true);
        cancelLaunchRef.current = launchDucklingClone(el, {
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="flex flex-col items-center gap-3 py-4">
          <button
            ref={logoRef}
            type="button"
            aria-label={t("header.ducklingLogo")}
          ...
          <div className="text-center">
            <p className="text-lg font-semibold">Duckling Browser</p>
            {systemInfo && (
              <>
