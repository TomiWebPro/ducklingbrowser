/**
 * Shared physics for the Duckling logo easter eggs: clones an element into a
...
export interface DucklingLaunchOptions {
 ...
 */
export function launchDucklingClone(
  el: HTMLElement,
  options: DucklingLaunchOptions = {},
): () => void {
  ...
  // The fallen duckling is a toy: it can be grabbed 1:1 and thrown, inheriting
  ...
  const onPointerDown = (e: PointerEvent) => {
    ...
    // Respect where the duckling was grabbed — no snap to center.
  ...
  const onPointerUp = (e: PointerEvent) => {
    ...
    // throws the duckling instead of dropping it.
    const now = performance.now();
  ...
  const animate = (time: number) => {
    ...
    if (!grabbed) {
      ...
      if (currentBottom >= floorY && vy > 0) {
        ...
        // A duckling dropped with no sideways speed would hop in place forever —
      ...
      // damping), and keep rolling. Left wall has no bounce — the duckling
