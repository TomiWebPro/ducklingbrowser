 ...
 * to duckling-sync; client `list()` calls strip it from results so it never
 * leaks into application data.
 */
const MANIFEST_KEY = ".duckling-sync-manifest";
...
export class SyncService implements OnModuleInit {
  ...
  async list(dto: ListRequestDto, ctx?: UserContext): Promise<ListResponseDto> {
    ...
    const objects = (response.Contents || [])
      // Don't leak duckling-sync's internal manifest object to clients.
