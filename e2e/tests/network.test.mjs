...
async function createExtensionsThroughUi(app) {
  ...
  await app.waitForText("Duckling E2E Fixture");
  ...
  const extension = extensions.find(
    (item) => item.name === "Duckling E2E Fixture",
  );
...
async function assertProxyWorkerLogsRedacted(app, settings) {
  const files = (await readdir(path.join(app.root, "tmp"))).filter(
    (file) => file.startsWith("duckling-proxy-") && file.endsWith(".log"),
  );
...
function wireGuardTargetWasReached() {
  const container = process.env.DUCKLING_E2E_WIREGUARD_CONTAINER;
  ...
  return (
    spawnSync(
      ...
      [
        ...
        "grep",
        "-q",
        "GET /duckling-e2e-wireguard ",
        "/tmp/duckling-e2e-target-requests",
      ],
...
test("visible UI creates and assigns profiles, groups, proxies, VPNs, extensions, and extension groups", async () => {
  ...
  const realWireGuardConfig = process.env.DUCKLING_E2E_WIREGUARD_CONFIG_BASE64
    ? Buffer.from(
        process.env.DUCKLING_E2E_WIREGUARD_CONFIG_BASE64,
        "base64",
      ).toString("utf8")
    : null;
  ...
  try {
    ...
    assert.equal(
      await app.visibleTextIncludes("Welcome to Duckling Browser"),
      false,
    ...
    if (realWireGuardConfig) {
      const tunneled = await runProfile(
        ...
        saved.api_token,
        profile.id,
        process.env.DUCKLING_E2E_WIREGUARD_TARGET_URL,
      );
