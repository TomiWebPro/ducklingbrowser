...
async function dragBackgroundColorPicker(app) {
  ...
  await app.execute(`
    window.__ducklingE2eThemePointerEvents = [];
    for (const type of ["pointermove", "pointerdown", "pointerup"]) {
      window.addEventListener(type, (event) => {
        window.__ducklingE2eThemePointerEvents.push({
          type,
          x: event.clientX,
          y: event.clientY,
  ...
  const pointerEvents = await app.execute(
    `return window.__ducklingE2eThemePointerEvents ?? [];`,
  );
  assert.deepEqual(
...
test("first-run onboarding stays recoverable, responsive, and platform-aware", async () => {
  await withApp(
    "ui-onboarding",
    async (app) => {
      await app.waitForText("Welcome to Duckling Browser");
