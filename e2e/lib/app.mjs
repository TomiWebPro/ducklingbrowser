...
function isolatedEnvironment(root, extra = {}) {
  ...
  return {
    DUCKLINGBROWSER_DATA_ROOT: path.join(root, "duckling"),
    HOME: home,
    USERPROFILE: home,
...
export class AppSession {
  ...
  get dataRoot() {
    return path.join(this.root, "duckling");
  }
  ...
  async start() {
    ...
    const env = isolatedEnvironment(this.root, {
      DUCKLING_E2E_DISABLE_STARTUP_NETWORK: "1",
      ...(process.env.DUCKLING_E2E_FIXTURE_URL
        ? {
            DUCKLING_E2E_DNS_BLOCKLIST_BASE_URL: `${process.env.DUCKLING_E2E_FIXTURE_URL}/dns`,
            ...(process.env.DUCKLING_E2E_GEOIP_FIXTURE_READY === "1"
              ? {
                  DUCKLING_E2E_GEOIP_DOWNLOAD_URL: `${process.env.DUCKLING_E2E_FIXTURE_URL}/geoip.mmdb`,
                }
              : {}),
          }
        : {}),
...
export function appFromEnvironment(name, options = {}) {
  const runRoot = process.env.DUCKLING_E2E_RUN_ROOT;
  assert.ok(runRoot, "DUCKLING_E2E_RUN_ROOT is required");
