# Duckling Browser native E2E tests
...
REST and MCP servers, two-device sync, S3 payload encryption, Chromium, CDP, and child-process
cleanup.
...
Install Duckling dependencies with `pnpm install`. The browser suite also needs
`CHROMIUM_TEST_TOKEN`. The runner reads it from the environment or Duckling's ignored `.env` without
...
Set `DUCKLING_E2E_CHROMIUM_PATH` to use a local browser fixture. Without it, the runner uses an ignored
...
Run everything with `pnpm e2e`. A normal run builds the Next frontend, `duckling-proxy`, the
private harness in `e2e/app`, and `tauri-wd`. The harness enables Duckling's `e2e` feature and injects
...
`DUCKLING_E2E_KEEP_ARTIFACTS=1` retains successful local runs; failed runs are always retained and
...
profile group, HTTP proxy, WireGuard VPN, extension, extension group, and Chromium profile; assigns
the proxy and VPN in the profile table; validates both residential HTTP and SOCKS5 proxies; then
launches Chromium through the residential proxy and through the local WireGuard tunnel. Normal test
sessions start with onboarding completed so the Welcome dialog cannot hide the feature under test.
The onboarding and Chromium-terms scenarios explicitly opt into fresh state and test those dialogs.
...
- Duckling data, cache, and logs with `DUCKLINGBROWSER_DATA_ROOT`;
...
exercise published Chromium downloads when no local fixture exists. Entitlement fallback from
`CHROMIUM_TEST_TOKEN` exists only in the feature-gated test binary. Production builds never include
...
including MinIO-backed sync and real Chromium automation, plus a Linux/Docker job for residential
...
job also requires the `CHROMIUM_TEST_TOKEN` secret. The network job requires that secret plus
