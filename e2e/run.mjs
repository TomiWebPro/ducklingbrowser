...
const appBinary = path.join(
  ...
  "target",
  "debug",
  `ducklingbrowser-e2e${executableSuffix}`,
);
...
function parseArgs(argv) {
  const options = {
    suite: "full",
    build: true,
    keep: process.env.DUCKLING_E2E_KEEP_ARTIFACTS === "1",
    verbose: process.env.DUCKLING_E2E_VERBOSE === "1",
  };
...
function log(message) {
  process.stdout.write(`[duckling-e2e] ${message}\n`);
}
...
function startProcess(name, command, args, { cwd, env, runRoot, verbose }) {
  ...
  child.on("error", (error) => {
    process.stderr.write(`[duckling-e2e] ${name} process error: ${error}\n`);
  });
...
function startFixtureServer(geoIpFixture) {
  const server = http.createServer((request, response) => {
    ...
    if (url.pathname === "/api/echo") {
      ...
      request.on("end", () => {
        response.writeHead(200, {
          ...
          "set-cookie": "duckling_e2e=browser-ok; Path=/; SameSite=Lax",
        });
        response.end(
          JSON.stringify({
    ...
    response.end(`<!doctype html>
      <html>
        <head><title>Duckling E2E Browser Fixture</title></head>
        <body>
          <h1 id="fixture-title">Duckling E2E Browser Fixture</h1>
...
async function ensureGeoIpFixture() {
  if (process.env.DUCKLING_E2E_GEOIP_FIXTURE) {
    const fixture = path.resolve(process.env.DUCKLING_E2E_GEOIP_FIXTURE);
    if (!existsSync(fixture)) {
      throw new Error(`DUCKLING_E2E_GEOIP_FIXTURE does not exist: ${fixture}`);
    }
    return fixture;
  }
  const toolsDir = path.join(os.tmpdir(), "duckling-e2e-tools");
  ...
  const releases = await fetch(
    ...
    {
      headers: { "user-agent": "duckling-browser-e2e" },
...
async function ensureMinio() {
  if (process.env.DUCKLING_E2E_MINIO_BIN) {
    return path.resolve(process.env.DUCKLING_E2E_MINIO_BIN);
  }
...
async function startSyncInfrastructure(runRoot, options, records) {
  ...
  const syncToken = "duckling-e2e-sync-token-0123456789abcdef";
  ...
  const syncRoot = path.join(projectRoot, "duckling-sync");
  ...
  const sync = startProcess("duckling-sync", "node", ["dist/main.js"], {
    cwd: syncRoot,
    runRoot,
    verbose: options.verbose,
    env: {
      ...
      S3_BUCKET: `duckling-e2e-${process.pid}`,
...
async function startWireGuardInfrastructure() {
  ...
  const name = `duckling-wg-e2e-${process.pid}-${Date.now()}`;
  const image =
    process.env.DUCKLING_E2E_WIREGUARD_IMAGE ??
  ...
  try {
    ...
    const server = runDocker([
      ...
      "sh",
      "-c",
      'while true; do printf "HTTP/1.1 200 OK\\r\\nContent-Length: 13\\r\\nConnection: close\\r\\n\\r\\nWG-TUNNEL-OK\\n" | nc -l -p 8080 >> /tmp/duckling-e2e-target-requests 2>/dev/null; done',
    ]);
    if (server.status !== 0) {
    ...
    return {
      name,
      config,
      targetUrl: "http://10.64.0.1:8080/duckling-e2e-wireguard",
    };
  } catch (error) {
...
async function main() {
  ...
  const runRoot = await mkdtemp(path.join(os.tmpdir(), "duckling-e2e-"));
  ...
  const sensitiveValues = [
    "duckling-e2e-sync-token-0123456789abcdef",
    "minioadmin",
  ];
  const cleanup = async () => {
  ...
  try {
    ...
    const networkEnabled =
      ...
      process.env.DUCKLING_E2E_SKIP_NETWORK_TEST !== "1";
    ...
    if (networkEnabled && process.env.DUCKLING_E2E_SKIP_VPN_TUNNEL !== "1") {
    ...
    const child = spawn(process.execPath, testArgs, {
      ...
      env: {
        ...process.env,
        DUCKLING_E2E_RUN_ROOT: runRoot,
        DUCKLING_E2E_PROJECT_ROOT: projectRoot,
        DUCKLING_E2E_WEBDRIVER_ROOT: webdriverRoot,
        DUCKLING_E2E_APP: appBinary,
        ...
        DUCKLING_E2E_SYNC_URL: sync.syncUrl ?? "",
        DUCKLING_E2E_SYNC_TOKEN: sync.syncToken ?? "",
        DUCKLING_E2E_MINIO_URL: sync.minioUrl ?? "",
        DUCKLING_E2E_WIREGUARD_CONFIG_BASE64: wireGuard
          ? Buffer.from(wireGuard.config).toString("base64")
          : "",
        DUCKLING_E2E_WIREGUARD_TARGET_URL: wireGuard?.targetUrl ?? "",
        DUCKLING_E2E_WIREGUARD_CONTAINER: wireGuard?.name ?? "",
      },
      stdio: "inherit",
    });
    ...
    failed = true;
    process.stderr.write(`[duckling-e2e] ERROR: ${error.stack ?? error}\n`);
    process.exitCode = 1;
  } finally {
    await cleanup();
  }
