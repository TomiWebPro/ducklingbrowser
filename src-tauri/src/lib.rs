  ...
  #[cfg(target_os = "macos")]
  {
    dirs::home_dir().map(|h| {
      h.join("Library")
        ...
        .join("local.mcpb.duckling-browser.duckling-browser")
    })
  }
...
async fn add_mcp_to_claude_desktop_internal(app_handle: &tauri::AppHandle) -> Result<(), String> {
  ...
  let manifest = serde_json::json!({
    ...
    "name": "duckling-browser",
    "display_name": "Duckling Browser",
    "version": env!("CARGO_PKG_VERSION"),
    "description": "Control Duckling Browser profiles, proxies, and automation via MCP",
    "author": { "name": "Duckling Browser" },
...
fn remove_mcp_from_claude_desktop_internal() -> Result<(), String> {
  ...
  update_claude_extensions_registry("local.mcpb.duckling-browser.duckling-browser", None)?;
  Ok(())
}
...
pub async fn check_vpn_validity_core(
  ...
  for attempt in 0..3 {
    ...
    match ip_utils::fetch_public_ip(Some(&local_proxy_url)).await {
      ...
      Err(error) => {
        log::warn!(
          "VPN validation attempt {} failed to fetch public IP through duckling-proxy: {}",
          attempt + 1,
          error
        );
      }
...
fn setup_system_tray(app: &tauri::AppHandle) -> Result<(), Box<dyn std::error::Error>> {
  ...
  let show_item = MenuItemBuilder::with_id("tray_show", "Show Duckling Browser").build(app)?;
  ...
  TrayIconBuilder::with_id("main")
    ...
    .on_tray_icon_event(|tray, event| {
      ...
      // restore via the "Show Duckling Browser" menu item.
...
pub fn run_with_builder(
  ...
  // Honor DUCKLINGBROWSER_DATA_ROOT: when set, logs go to <root>/logs instead of
  ...
  builder.setup(|app| {
      ...
      let win_builder = match e2e_automation_profile_dir() {
          Some(profile_dir) => win_builder
            ...
            // DUCKLINGBROWSER_DATA_ROOT; only browser-engine storage is ephemeral.
            .incognito(true),
          None => win_builder,
      };
      ...
      // duckling-proxy workers on disk must be orphans the current app can't track.
