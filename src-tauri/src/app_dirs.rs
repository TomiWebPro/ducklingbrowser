...
/// `DUCKLINGBROWSER_DATA_ROOT=/path` (e.g. a tmpfs mount) to relocate
/// data/cache/logs under `<root>/{data,cache,logs}` without touching the real
/// dev/prod directories. The more specific `DUCKLINGBROWSER_DATA_DIR` /
/// `DUCKLINGBROWSER_CACHE_DIR` overrides still take precedence over this.
fn data_root() -> Option<PathBuf> {
  std::env::var_os("DUCKLINGBROWSER_DATA_ROOT")
    .filter(|v| !v.is_empty())
    .map(PathBuf::from)
}

/// Log directory when `DUCKLINGBROWSER_DATA_ROOT` is set (`<root>/logs`); `None`
...
pub fn data_dir() -> PathBuf {
  ...
  if let Ok(dir) = std::env::var("DUCKLINGBROWSER_DATA_DIR") {
...
pub fn cache_dir() -> PathBuf {
  ...
  if let Ok(dir) = std::env::var("DUCKLINGBROWSER_CACHE_DIR") {
    return PathBuf::from(dir);
  }
...
mod tests {
  ...
  fn test_app_name() {
    let name = app_name();
    assert!(
      name == "DucklingBrowser" || name == "DucklingBrowserDev",
      "app_name should be DucklingBrowser or DucklingBrowserDev, got: {name}"
    );
  }
  ...
  fn test_set_test_data_dir() {
    let tmp = PathBuf::from("/tmp/test-duckling-data");
  ...
  fn test_set_test_cache_dir() {
    let tmp = PathBuf::from("/tmp/test-duckling-cache");
