...
/// one as a prefix). Mirrors the precise matching in browser_runner/chromium_manager.
///
...
pub mod macos {
  ...
  pub async fn open_url_in_existing_browser_chromium(
    ...
    // and trigger the macOS TCC "<Duckling> wants control of <Browser>" /
    // "prevented from modifying other apps" prompts. Duckling must never touch
