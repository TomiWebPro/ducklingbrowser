...
This module provides comprehensive self-update functionality for the Duckling Browser application
...
impl AppAutoUpdater {
  ...
  pub async fn check_for_updates(
    ...
    if self.should_update(&current_version, &latest_release.tag_name, is_nightly) {
      ...
      let release_page_url = format!(
        "https://github.com/TomiWebPro/ducklingbrowser/releases/tag/{}",
        latest_release.tag_name
      );
  ...
  async fn fetch_app_releases(
    ...
    let url = "https://api.github.com/repos/TomiWebPro/ducklingbrowser/releases?per_page=100";
    let response = self
      .client
      .get(url)
  ...
  fn is_deb_repo_configured() -> bool {
    Path::new("/etc/apt/sources.list.d/ducklingbrowser.list").exists()
  }

  /// Check if an RPM repository is configured (yum/dnf or zypper)
  #[cfg(target_os = "linux")]
  fn is_rpm_repo_configured() -> bool {
    Path::new("/etc/yum.repos.d/ducklingbrowser.repo").exists()
      || Path::new("/etc/zypp/repos.d/ducklingbrowser.repo").exists()
  }
  ...
  pub async fn download_and_prepare_update(
    ...
    let temp_dir = std::env::temp_dir().join("duckling_app_update");
  ...
  async fn install_update(
    ...
    #[cfg(target_os = "macos")]
    {
      ...
      // Clean up old "Duckling Browser.app" if it exists (from before the project rename)
      if let Some(parent_dir) = current_app_path.parent() {
        let old_app_path = parent_dir.join("Duckling Browser.app");
        if old_app_path.exists() && old_app_path != current_app_path {
          log::info!(
            "Removing old 'Duckling Browser.app' from: {}",
            old_app_path.display()
          );
          if let Err(e) = fs::remove_dir_all(&old_app_path) {
            log::warn!("Warning: Failed to remove old 'Duckling Browser.app': {e}");
          } else {
            log::info!("Successfully removed old 'Duckling Browser.app'");
          }
        }
      }
  ...
  async fn prepare_windows_installer() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let remaining: Vec<u32> = worker_pids
      ...
      .collect();
    log::error!(
      "App update aborted because duckling-proxy worker PIDs are still running: {:?}",
      remaining
    );
    Err(
      serde_json::json!({
  ...
  async fn restart_application(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    #[cfg(target_os = "macos")]
    {
      ...
      let script_path = temp_dir.join("duckling_restart.sh");
...
"#,
    ...
    #[cfg(target_os = "windows")]
    {
      ...
      if let Some(installer_path) = pending {
        ...
        let current_pid = std::process::id();
        let temp_dir = std::env::temp_dir();
        let script_path = temp_dir.join("duckling_restart.bat");
...
pub async fn check_for_app_updates() -> Result<Option<AppUpdateInfo>, String> {
  ...
  if crate::e2e_automation_enabled()
    && std::env::var_os("DUCKLING_E2E_DISABLE_STARTUP_NETWORK").is_some()
  {
...
mod tests {
  ...
  fn test_find_checksum_for_file() {
    let sums = "\
0e5a4601745092b7d1c93c1e7e1c30d923be3d1e916b661bd53d1c0c9c7f0a11  Duckling_0.29.0_aarch64.dmg
ABCDEF01745092B7D1C93C1E7E1C30D923BE3D1E916B661BD53D1C0C9C7F0A22 *Duckling_0.29.0_x64.dmg
not-a-hash  Duckling_0.29.0_amd64.deb
";

    // Plain entry.
    assert_eq!(
      AppAutoUpdater::find_checksum_for_file(sums, "Duckling_0.29.0_aarch64.dmg").as_deref(),
    ...
    assert_eq!(
      AppAutoUpdater::find_checksum_for_file(sums, "Duckling_0.29.0_x64.dmg").as_deref(),
    ...
    assert_eq!(
      AppAutoUpdater::find_checksum_for_file(sums, "Duckling_0.29.0_amd64.deb"),
      None
    );
    // Missing file.
    assert_eq!(
      AppAutoUpdater::find_checksum_for_file(sums, "Duckling_0.29.0_arm64.deb"),
      None
    );
  }
  ...
  fn test_find_checksums_url() {
    let assets = vec![
      AppReleaseAsset {
        name: "Duckling_0.29.0_x64.dmg".to_string(),
  ...
  fn test_windows_installer_hook_protects_sidecar_replacement() {
    ...
    assert!(hooks.contains("IfFileExists \"$INSTDIR\\duckling-proxy.exe\""));
    assert!(hooks.contains("taskkill.exe"));
    assert!(hooks.contains("duckling-proxy.exe"));
    assert!(hooks.contains("Delete \"$INSTDIR\\duckling-proxy.exe\""));
  }
  ...
  fn test_platform_specific_download_urls() {
    ...
    let all_assets = vec![
      // macOS assets
      AppReleaseAsset {
        name: "Duckling.Browser_0.1.0_aarch64.dmg".to_string(),
        browser_download_url: "https://example.com/aarch64.dmg".to_string(),
        size: 12345,
        digest: None,
      },
      AppReleaseAsset {
        name: "Duckling.Browser_0.1.0_x64.dmg".to_string(),
        browser_download_url: "https://example.com/x64.dmg".to_string(),
        size: 12345,
        digest: None,
      },
      ...
      AppReleaseAsset {
        name: "Duckling_0.1.0_x64-setup.exe".to_string(),
      ...
      // Linux assets
      AppReleaseAsset {
        name: "ducklingbrowser_0.1.0_amd64.deb".to_string(),
        browser_download_url: "https://example.com/amd64.deb".to_string(),
        size: 12345,
        digest: None,
      },
      AppReleaseAsset {
        name: "ducklingbrowser-0.1.0-1.x86_64.rpm".to_string(),
        browser_download_url: "https://example.com/x86_64.rpm".to_string(),
        size: 12345,
        digest: None,
      },
      AppReleaseAsset {
        name: "Duckling.Browser-0.1.0-x86_64.AppImage".to_string(),
  ...
  fn test_appimage_detection_logic() {
    ...
    let all_assets = vec![
      ...
      // Windows assets
      AppReleaseAsset {
        name: "Duckling.Browser_0.1.0_x64.msi".to_string(),
