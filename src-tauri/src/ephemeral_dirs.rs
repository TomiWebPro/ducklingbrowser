...
fn get_ephemeral_base_dir() -> Result<PathBuf, String> {
  #[cfg(target_os = "linux")]
  {
    let base = PathBuf::from("/dev/shm/duckling-ephemeral");
  ...
  #[cfg(not(target_os = "linux"))]
  {
    ...
    // Fallback
    let base = std::env::temp_dir().join("duckling-ephemeral");
    std::fs::create_dir_all(&base)
...
fn get_or_create_macos_ramdisk() -> Result<PathBuf, String> {
  let mount_point = PathBuf::from("/Volumes/DucklingEphemeral");
  ...
  let fmt = std::process::Command::new("diskutil")
    .args(["erasevolume", "HFS+", "DucklingEphemeral", &dev])
    .output()
...
fn get_or_create_windows_ramdisk() -> Result<PathBuf, String> {
  ...
  for letter in ['R', 'Q', 'P', 'O'] {
    let base = PathBuf::from(format!("{}:\\DucklingEphemeral", letter));
  ...
  for letter in ['R', 'Q', 'P', 'O'] {
    ...
    match output {
      Ok(out) if out.status.success() => {
        let base = PathBuf::from(format!("{}\\DucklingEphemeral", drive));
        std::fs::create_dir_all(&base)
...
fn cleanup_legacy_dirs() {
  ...
  for entry in entries.flatten() {
    if let Some(name) = entry.file_name().to_str() {
      if name.starts_with("duckling-ephemeral-") && entry.path().is_dir() {
