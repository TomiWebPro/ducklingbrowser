...
use ducklingbrowser_lib::proxy_runner::{
  start_proxy_process_with_profile, stop_all_proxy_processes, stop_proxy_process,
};
use ducklingbrowser_lib::proxy_server::{redacted_upstream, run_proxy_server};
use ducklingbrowser_lib::proxy_storage::{build_proxy_url, get_proxy_config};
use std::process;
...
async fn main() {
  ...
  // RUST_LOG=debug or RUST_LOG=duckling_proxy=trace still works for deep dives.
  ...
  let matches = Command::new("duckling-proxy")
  ...
  if let Some(proxy_matches) = matches.subcommand_matches("proxy") {
    if let Some(start_matches) = proxy_matches.subcommand_matches("start") {
      ...
      if let (Some(host), Some(port), Some(proxy_type)) = (
        ...
        let username = std::env::var("DUCKLING_PROXY_USERNAME").ok();
        let password = std::env::var("DUCKLING_PROXY_PASSWORD").ok();
      ...
      if let Some(id) = stop_matches.get_one::<String>("id") {
        ...
        let configs = ducklingbrowser_lib::proxy_storage::list_proxy_configs();
    ...
    if action == "start" {
      ...
      let config = if let Some(path) = config_path {
        ...
        match std::fs::read_to_string(path) {
          Ok(content) => match serde_json::from_str::<
            ducklingbrowser_lib::vpn_worker_storage::VpnWorkerConfig,
          >(&content)
          {
            Ok(config) => {
              log::info!(
        ...
        let storage_dir = ducklingbrowser_lib::proxy_storage::get_storage_dir();
        log::info!("Looking for VPN worker config in: {:?}", storage_dir);
        let mut attempts = 0;
        loop {
          if let Some(config) = ducklingbrowser_lib::vpn_worker_storage::get_vpn_worker_config(id) {
            log::info!(
              "Found VPN worker config: id={}, vpn_type={}, vpn_id={}",
              config.id,
              config.vpn_type,
      ...
      match config.vpn_type.as_str() {
        "wireguard" => {
          let wg_config = match ducklingbrowser_lib::vpn::parse_wireguard_config(&vpn_config_data) {
            Ok(c) => c,
            Err(e) => {
          ...
          let server =
            ducklingbrowser_lib::vpn::socks5_server::WireGuardSocks5Server::new(wg_config, port);
          if let Err(e) = server
