// MCP client integrations — installs/removes the duckling-browser MCP server in
...
pub struct McpAgentInfo {
  ...
  /// the duckling-browser server into it.
  pub detected: bool,
}
...
/// Transform the duckling-browser HTTP server config into the per-agent shape.
...
/// Detect whether a server config object looks like our duckling-browser HTTP
/// endpoint by URL prefix. Matches across the various per-agent key shapes
/// (`url`, `uri`, `serverUrl`).
fn config_matches_duckling(value: &serde_json::Value) -> bool {
...
fn is_generic_agent_connected(agent_id: &str) -> bool {
  ...
  if let Some(entry) = servers.get(SERVER_NAME) {
    return config_matches_duckling(entry);
  }
  servers.values().any(config_matches_duckling)
}

/// Install or remove the duckling-browser entry from a generic agent. Returns
