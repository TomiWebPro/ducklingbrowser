...
mod tests {
  ...
  #[cfg(target_os = "macos")]
  const SYNTHETIC_OS_CRYPT_PASSWORD: &[u8] = b"duckling-synthetic-cookie-key";
  #[cfg(target_os = "macos")]
  ...
  fn test_decrypt_v10_cookie_with_synthetic_vector() {
    let profile_dir =
      std::env::temp_dir().join(format!("duckling_decrypt_vector_{}", uuid::Uuid::new_v4()));
  ...
  fn test_create_empty_chrome_cookies_db_then_write() {
    let dir = std::env::temp_dir().join(format!("duckling_empty_chrome_{}", uuid::Uuid::new_v4()));
