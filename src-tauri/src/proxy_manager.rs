...
impl ProxyManager {
  ...
  // Check if a proxy is valid by routing through a temporary duckling-proxy process.
  ...
  pub fn export_proxies_json(&self) -> Result<String, String> {
    ...
    let export_data = ProxyExportData {
      ...
      source: "DucklingBrowser".to_string(),
    };
  ...
  pub async fn start_proxy(
    ...
    // Start a new proxy using the duckling-proxy binary with the correct CLI interface
    let mut proxy_cmd = app_handle
      .shell()
      .sidecar("duckling-proxy")
    ...
    if let Some(proxy_settings) = proxy_settings {
      ...
      if let Some(username) = &proxy_settings.username {
        proxy_cmd = proxy_cmd.env("DUCKLING_PROXY_USERNAME", username);
      }
      if let Some(password) = &proxy_settings.password {
        proxy_cmd = proxy_cmd.env("DUCKLING_PROXY_PASSWORD", password);
      }
    }
    ...
    // The duckling-proxy binary should start the worker and then exit
    let output = proxy_cmd
      .output()
      .await
      .map_err(|e| format!("Failed to execute duckling-proxy: {e}"))?;
  ...
  pub async fn stop_proxy(
    ...
    // Stop the proxy using the duckling-proxy binary
    ...
    match proxy_cmd.output().await {
      ...
      Ok(_) => {}
      Err(e) => log::warn!("Failed to run duckling-proxy stop: {e}"),
    }
  ...
  pub async fn cleanup_dead_proxies(
    ...
    // X button (bypassing Duckling's stop flow) or the browser crashes, the
    // worker keeps running forever. On Windows users reported dozens of
    // duckling-proxy processes accumulating this way.
    {
...
mod tests {
  ...
  // Integration test that actually builds and uses duckling-proxy binary
  #[tokio::test]
  async fn test_proxy_integration_with_real_proxy() -> Result<(), Box<dyn std::error::Error>> {
    // This test requires duckling-proxy binary to be available
