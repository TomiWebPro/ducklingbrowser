...
// Cloud backend not available in this standalone fork
...
impl CloudAuthManager {
  fn new() -> Self {
    ...
    // the startup chain (sync-token → proxy-config).
  ...
  /// Get the current Chromium token, if any. Always returns None in this fork.
