...
fn unsuffixed_binary_name(base_name: &str) -> String {
  ...
  {
    match base_name {
      "duckling-proxy" => "duckling-proxy.exe".to_string(),
      _ => String::new(),
    }
  }
...
pub(crate) async fn ensure_sidecar_version() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let executable = match find_sidecar_executable("duckling-proxy") {
    Ok(executable) => executable,
    Err(e) => {
      log::error!("Failed to locate duckling-proxy for version verification: {e}");
  ...
  log::error!(
    "duckling-proxy version mismatch: expected {}, got {:?}; status={}, stdout={:?}, stderr={:?}",
    expected_version,
    actual_version,
    output.status,
...
pub async fn start_proxy_process_with_profile(
  ...
  let log_path = temp_dir.join(format!("duckling-proxy-{id}.log"));
  ...
  #[cfg(unix)]
  {
    ...
    cmd.arg("--id");
    cmd.arg(&id);
    cmd.env_remove("DUCKLING_PROXY_USERNAME");
    cmd.env_remove("DUCKLING_PROXY_PASSWORD");
...
mod tests {
  ...
  fn parses_exact_sidecar_version_output() {
    assert_eq!(
      parse_sidecar_version(b"duckling-proxy v0.28.2\n").as_deref(),
      Some("v0.28.2")
    );
    assert_eq!(
      parse_sidecar_version(b"duckling-proxy nightly-2026-07-19-a4ed5c8\r\n").as_deref(),
  ...
  fn rejects_missing_or_ambiguous_sidecar_version_output() {
    ...
    assert_eq!(parse_sidecar_version(b"duckling-proxy"), None);
  ...
  fn prunes_only_old_proxy_logs() {
    ...
    for id in ["oldest", "middle", "newest"] {
      fs::write(temp.path().join(format!("duckling-proxy-{id}.log")), id).unwrap();
    ...
    assert!(!temp.path().join("duckling-proxy-oldest.log").exists());
    assert!(temp.path().join("duckling-proxy-middle.log").exists());
    assert!(temp.path().join("duckling-proxy-newest.log").exists());
