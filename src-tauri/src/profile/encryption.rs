...
const VERIFY_FILE_NAME: &str = ".duckling-pw-verify";
const VERIFY_FILE_PATH: &str = "__duckling_pw_verify__";
...
fn atomic_write(path: &Path, data: &[u8]) -> std::io::Result<()> {
  ...
  let tmp = path.with_extension("duckling-tmp");
  std::fs::write(&tmp, data)?;
  std::fs::rename(&tmp, path)
}

fn write_verifier(key: &[u8; 32], encrypted_dir: &Path) -> PasswordResult<()> {
  let encrypted = encrypt_profile_file(key, VERIFY_FILE_PATH, b"duckling-verify")?;
...
pub fn verify_key_against_dir(key: &[u8; 32], encrypted_dir: &Path) -> PasswordResult<()> {
  ...
  if relpath != VERIFY_FILE_PATH || content != b"duckling-verify" {
    return Err(PasswordError::InvalidFormat);
  }
  Ok(())
}
...
mod tests {
  ...
  fn test_atomic_write_leaves_original_intact_if_tmp_lingers() {
    ...
    std::fs::write(target.with_extension("duckling-tmp"), b"partial").unwrap();
