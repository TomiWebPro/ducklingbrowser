fn main() {
  ...
  if let Ok(vault_password) = std::env::var("DUCKLING_BROWSER_VAULT_PASSWORD") {
    println!("cargo:rustc-env=DUCKLING_BROWSER_VAULT_PASSWORD={vault_password}");
  } else {
    // Use default password if environment variable is not set
    println!("cargo:rustc-env=DUCKLING_BROWSER_VAULT_PASSWORD=ducklingbrowser-api-vault-password");
  }
