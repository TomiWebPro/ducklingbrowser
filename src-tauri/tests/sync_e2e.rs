use ducklingbrowser_lib::sync::types::*;
use reqwest::Client;
use serde_json::json;
use std::env;
use std::fs;
    ...
    Ok(response) => {
      if !response.status().is_success() {
        panic!(
          ...
          Server URL: {}\n\
          Please ensure:\n\
          1. MinIO is running (docker compose up -d in duckling-sync/)\n\
          2. duckling-sync server is running (cd duckling-sync && pnpm start:dev)\n\
...
async fn test_encrypted_profile_sync() {
  use ducklingbrowser_lib::sync::encryption::{
