...
/// Setup function to ensure duckling-proxy binary exists and cleanup stale proxies
async fn setup_test() -> Result<std::path::PathBuf, Box<dyn std::error::Error + Send + Sync>> {
  ...
  // Build duckling-proxy binary if it doesn't exist
      ...
      .is_ok_and(|output| {
        output.status.success()
          && String::from_utf8_lossy(&output.stdout).trim()
            == format!("duckling-proxy {}", env!("BUILD_VERSION"))
      });

  if !binary_is_current {
    println!("Building duckling-proxy binary for integration tests...");
...
async fn test_traffic_tracking() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  let traffic_stats_dir = ducklingbrowser_lib::app_dirs::cache_dir().join("traffic_stats");
...
async fn test_bypass_rules_http_direct() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  // Start duckling-proxy with upstream + bypass rules for "127.0.0.1"
  ...
  println!("Duckling-proxy started on port {local_port} with bypass rules for 127.0.0.1");
...
async fn test_bypass_rules_in_config() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  let proxies_dir = ducklingbrowser_lib::app_dirs::proxy_workers_dir();
...
/// Test that a SOCKS5 upstream proxy works end-to-end through duckling-proxy.
/// Starts a mock SOCKS5 server, a mock HTTP target server,
/// then routes requests through duckling-proxy -> SOCKS5 -> target.
#[tokio::test]
#[serial]
async fn test_local_proxy_with_socks5_upstream(
  ...
  // Test 1: HTTP request through duckling-proxy -> SOCKS5 -> target
...
/// Verifies the full chain: client → duckling-proxy → Shadowsocks → internet.
#[tokio::test]
#[serial]
async fn test_local_proxy_with_shadowsocks_upstream(
  ...
  let ss_container = "duckling-ss-test";
  let ss_port = 18388u16;
  let ss_password = "duckling-test-password";
  ...
  // Start duckling-proxy with Shadowsocks upstream
  let output = tokio::process::Command::new(&binary_path)
    ...
    .env("DUCKLING_PROXY_USERNAME", ss_method)
    .env("DUCKLING_PROXY_PASSWORD", ss_password)
    .output()
    .await?;
  ...
  // Test: HTTP request through duckling-proxy → Shadowsocks → example.com
  let mut stream = TcpStream::connect(("127.0.0.1", local_port)).await?;
  let request =
