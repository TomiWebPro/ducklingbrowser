...
async fn test_wireguard_tunnel_init() {
  ...
  use ducklingbrowser_lib::vpn::{VpnTunnel, WireGuardTunnel};
...
async fn test_tunnel_manager() {
  use ducklingbrowser_lib::vpn::{TunnelManager, VpnStatus, VpnTunnel};
  ...
  impl VpnTunnel for MockTunnel {
    async fn connect(&mut self) -> Result<(), ducklingbrowser_lib::vpn::VpnError> {
      self.connected = true;
      Ok(())
    }

    async fn disconnect(&mut self) -> Result<(), ducklingbrowser_lib::vpn::VpnError> {
...
impl TestEnvGuard {
  fn new() -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
    ...
    let root = TEST_RUNTIME_ROOT
      .get_or_init(|| {
        std::env::temp_dir().join(format!("ducklingbrowser-vpn-e2e-{}", std::process::id()))
      })
      .clone();
    ...
    let previous_data_dir = std::env::var("DUCKLINGBROWSER_DATA_DIR").ok();
    let previous_cache_dir = std::env::var("DUCKLINGBROWSER_CACHE_DIR").ok();

    std::env::set_var("DUCKLINGBROWSER_DATA_DIR", &data_dir);
    std::env::set_var("DUCKLINGBROWSER_CACHE_DIR", &cache_dir);
...
impl Drop for TestEnvGuard {
  fn drop(&mut self) {
    if let Some(value) = &self.previous_data_dir {
      std::env::set_var("DUCKLINGBROWSER_DATA_DIR", value);
    } else {
      std::env::remove_var("DUCKLINGBROWSER_DATA_DIR");
    }

    if let Some(value) = &self.previous_cache_dir {
      std::env::set_var("DUCKLINGBROWSER_CACHE_DIR", value);
    } else {
      std::env::remove_var("DUCKLINGBROWSER_CACHE_DIR");
    }
  }
}
...
async fn cleanup_runtime() {
  let _ = ducklingbrowser_lib::proxy_runner::stop_all_proxy_processes().await;
  let _ = ducklingbrowser_lib::vpn_worker_runner::stop_all_vpn_workers().await;
...
async fn run_proxy_feature_suite(
  ...
  let vpn_worker = ducklingbrowser_lib::vpn_worker_runner::start_vpn_worker(vpn_id)
    .await
  ...
  assert!(
    ...
    "HTTP traffic through duckling-proxy+VPN tunnel should succeed, got: {}",
    &http_response[..http_response.len().min(300)]
  );

  let stats_file = ducklingbrowser_lib::app_dirs::cache_dir()
    .join("traffic_stats")
  ...
  assert!(
    ...
    "Bypass rules should still work when duckling-proxy is chained to a VPN worker"
  );
  stop_proxy(binary_path, &bypass_proxy.id).await?;
  bypass_server.abort();

  ducklingbrowser_lib::vpn_worker_runner::stop_vpn_worker(&vpn_worker.id)
    .await
    .map_err(|error| error.to_string())?;
  Ok(())
}
...
#[tokio::test]
#[serial]
async fn test_wireguard_traffic_flows_through_duckling_proxy(
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  let binary_path = ensure_duckling_proxy_binary().await?;
  ...
  {
    let storage = ducklingbrowser_lib::vpn::VPN_STORAGE.lock().unwrap();
