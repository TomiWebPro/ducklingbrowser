 ...
 * 2. Builds and starts duckling-sync server
...
async function buildDucklingSync() {
  log("Building duckling-sync...");
  ...
  const syncDir = path.join(ROOT_DIR, "duckling-sync");
  ...
  if (!existsSync(path.join(syncDir, "dist", "main.js"))) {
    throw new Error("duckling-sync build did not produce dist/main.js");
  }
  log("duckling-sync built");
}

async function startDucklingSync() {
  log(`Starting duckling-sync on port ${SYNC_PORT}...`);

  const proc = spawn("node", ["dist/main.js"], {
    cwd: path.join(ROOT_DIR, "duckling-sync"),
    env: {
      ...process.env,
  ...
  proc.stdout.on("data", (data) => {
    if (process.env.VERBOSE) {
      console.log(`[duckling-sync] ${data.toString().trim()}`);
    }
  });

  proc.stderr.on("data", (data) => {
    if (process.env.VERBOSE) {
      console.error(`[duckling-sync] ${data.toString().trim()}`);
    }
  });

  proc.on("error", (err) => {
    error(`duckling-sync error: ${err.message}`);
  });

  await waitForHealth(`http://localhost:${SYNC_PORT}/health`, 30000);
  log("duckling-sync is ready");
...
async function main() {
  ...
  try {
    ...
    await buildDucklingSync();
    await startDucklingSync();
